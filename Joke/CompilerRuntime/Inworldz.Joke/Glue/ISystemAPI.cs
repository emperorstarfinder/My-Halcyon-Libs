...
namespace Inworldz.Joke.Glue
{
	public interface ISystemAPI
	{
        ...
        LSLList wcParseString2List(string src, LSLList separators, LSLList spacers, LSLList args);
		...
		void wcMakeNotecard(string name,LSLList data);
		void wcAvatarName2Key(string firstName,string lastName);
        void wcLinkTargetOmega(int linknumber, Vector3 axis, float spinrate, float gain);
        int llSetRegionPos(Vector3 position);
        int wcGetLinkInventoryNumber(int linknumber, int type);
        int wcGetLinkInventoryType(int linknumber, string name);
        int wcGetLinkInventoryPermMask(int linknumber, string item, int mask);
        string wcGetLinkInventoryName(int linknumber, int type, int number);
        string wcGetLinkInventoryKey(int linknumber, string name);
        string wcGetLinkInventoryCreator(int linknumber, string item);
        string wcSHA256String(string src);
        void wcTeleportAgent(string agent, string region, Vector3 pos, Vector3 lookat);
        string llAvatarOnLinkSitTarget(int linknumber);
        string wcGetLastOwner();
        void wcRemoveLinkInventory(int linknumber, string item);
        void wcGiveLinkInventory(int linknumber, string destination, string inventory);
        void wcGiveLinkInventoryList(int linknumber, string target, string folder, LSLList inventory);
        string wcGetNotecardSegment(string name, int line, int startOffset, int maxLength);
        string wcGetLinkNumberOfNotecardLines(int linknumber, string name);
        string wcGetLinkNotecardLine(int linknumber, string name, int line);
        string wcGetLinkNotecardSegment(int linknumber, string name, int line, int startOffset, int maxLength);
        int wcActiveGroup(string agent, string group);
        string wcAvatarOnLink(int linknumber);
        void llRegionSayTo(string destId, int channel, string msg);
        string wcGetLinkInventoryDesc(int linknumber, string name);
        string llGenerateKey();
        string wcGetLinkInventoryLastOwner(int linknumber, string name);
        ...
        void wcRezObject(string inventory, Vector3 pos, Vector3 vel, Quaternion rot, int param);
        void wcRezAtRoot(string inventory, Vector3 pos, Vector3 vel, Quaternion rot, int param);
        string wcRezPrim(LSLList primParams, LSLList particleSystem, LSLList inventory, Vector3 pos, Vector3 vel, Quaternion rot, int param);
        ...
        void wcSetWind(int type, Vector3 offset, Vector3 speed);
        Vector3 wcWind(Vector3 offset);
        int wcHasParcelPowers(int groupPower);
        Vector3 wcGroundSurfaceNormal(Vector3 offset);
        string wcRequestAnimationData(string name);
        LSLList llCastRay(Vector3 start, Vector3 end, LSLList options);
        void llSetKeyframedMotion(LSLList keyframes, LSLList options);
        int wcGetLocalTime();
        int wcGetLocalTimeOffset();
        string wcFormatTime(int unixtime, int isUTC, string format);
        int wcCheckRezError(Vector3 pos, int isTemp, int landImpact);
        ...
        int wcChar2Int(string src, int index);
        string wcInt2Char(int num);
        string wcReplaceString(string str, string pattern, string replacement);
        string wcFormatString(string str, LSLList values);
        int wcMatchString(string str, string pattern, int matchType);
        string wcStringCodec(string str, string pattern, int operation, LSLList extraParams);
        int wcMatchList(LSLList list1, LSLList list2, int matchType);
        Vector3 wcColorConvert(Vector3 input, int color1, int color2);
        Vector3 wcNameToColor(string name);
        int wcVerifyType(string str, int type);
        int wcGroupInvite(string group, string user, string role);
        int wcGroupEject(string group, string user);
        string wcGetAgentData(string id, int data);
        int wcIsPlusUser(string id);
        void llAttachToAvatarTemp(int attachPoint);
        int wcListIncludesElements(LSLList src, LSLList elements, int any);
        string wcReverseString(string src);
        LSLList wcReverseList(LSLList src, int stride);
        LSLList wcSearchInventory(int type, string pattern, int matchtype);
        LSLList wcSearchLinkInventory(int link, int type, string pattern, int matchtype);
        int wcIntRand(int max);
        int wcIntRandRange(int min, int max);
        float wcFrandRange(float min, float max);
        void botSearchBotOutfits(string pattern, int matchType, int start, int end);//Returns LSLList via async return
        LSLList wcListRemoveElements(LSLList src, LSLList elements, int count, int mode);
        LSLList wcListRemoveDuplicates(LSLList src);
        void wcStartLinkAnimation(int link, string anim);
        void wcStopLinkAnimation(int link, string anim);
        int wcClampInt(int value, int min, int max);
        float wcClampFloat(float value, float min, float max);
        LSLList wcSearchLinksByName(string pattern, int matchType, int linksOnly);
        LSLList wcSearchLinksByDesc(string pattern, int matchType, int linksOnly);
        int botHasTag(string botID, string tag);
        LSLList botGetBotTags(string botID);
        int wcValidateURL(string url);
        int wcRemoteLoadScriptPin(string target,string name,int pin,int running,int start_param);
        void wcDeliverInventory(int linknumber, string destination, string inventory);
        void wcDeliverInventoryList(int linknumber, string target, string folder, LSLList inventory);
        ...
        string wcGiveMoney(string destination, int amount);
        void wcStandTarget(Vector3 offset, Quaternion rot);
        void wcLinkStandTarget(int link, Vector3 offset, Quaternion rot);
